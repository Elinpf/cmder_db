...
def doit(command):
    os.system(
        "terminator --new-tab -x 'echo \"{}\"; {}; $SHELL'".format(command, command))
    print("{}\n".format(command))
    return
...
> """)
    ...
    command = "wfuzz -L -c -z file,/usr/share/wfuzz/wordlist/Injections/SQL.txt -d \"{}\"  {}".format(
        q3, q2)
    doit(command)
...
> """)
        if q3 not in ["v", "t", "b"]:
            ...
            break
    if q3 == "v":
        command = 'wfuzz -c -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt -u {}'.format(
            q2)
    elif q3 == "t":
        command = 'wfuzz -c -w /usr/share/seclists/Fuzzing/LFI/LFI-LFISuite-pathtotest.txt -u {}'.format(
            q2)
    elif q3 == "b":
        command = 'wfuzz -c -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt -w /usr/share/seclists/Fuzzing/LFI/LFI-LFISuite-pathtotest.txt -u {}'.format(
            q2)
    doit(command)
...
> """)
    command = 'wfuzz  -w /usr/share/wordlists/dirb/common.txt --hc 404,500 -u {}'.format(
        q2)
    doit(command)
