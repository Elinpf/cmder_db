...
prekeys = [binascii.unhexlify("308201130201010420"),
...
def create_env(db_dir):
    db_env = DBEnv(0)
    r = db_env.open(db_dir, (DB_CREATE | DB_INIT_LOCK | DB_INIT_LOG |
...
def first_read(device, size, prekeys, inc=10000):
    ...
    try:
        ...
        sys.stdout.write(
            "Can't open %s, check the path or try as root" % device)
        exit(0)
    prekey = prekeys[0]
    data = ""
    i = 0
...
class KEY:
    ...
    def generate(self, secret=None):
        if secret:
            ...
            self.prikey = ecdsa.SigningKey.from_secret_exponent(
                exp, curve=secp256k1)
        else:
    ...
    def set_privkey(self, key):
        if len(key) == 279:
            ...
            self.prikey = ecdsa.SigningKey.from_string(
                octet_str, curve=secp256k1)
        else:
    ...
    def get_privkey(self):
        ...
        encoded_gxgy = "\x04" + \
            ("%64x" % _Gx).decode('hex') + ("%64x" % _Gy).decode('hex')
...
def parse_wallet(db, item_callback):
    ...
    for (key, value) in db.items():
        ...
        try:
            ...
            sys.stdout.write("value data in hex: %s" %
...
def read_wallet(json_db, walletfile, print_wallet, print_wallet_transactions, transaction_filter, include_balance, vers=-1, FillPool=False):
    ...
    def item_callback(type, d):
        if type == "tx":
            json_db['tx'].append({"tx_id": d['tx_id'], "txin": d['txIn'],
                                  "txout": d['txOut'], "tx_v": d['txv'], "tx_k": d['txk']})
            ...
            json_db['keys'].append({'addr': addr, 'sec': sec, 'hexsec': hexsec, 'secret': hexsec, 'pubkey': d['public_key'].encode(
                'hex'), 'compressed': compressed, 'private': d['private_key'].encode('hex')})
            ...
            """     d['n'] = kds.read_int64()
                    ...
                    d['public_key'] = vds.read_bytes(vds.read_compact_size())"""
            try:
                json_db['pool'].append({'n': d['n'], 'addr': public_key_to_bc_address(d['public_key']), 'addr2': public_key_to_bc_address(d['public_key'].decode(
                    'hex')), 'addr3': public_key_to_bc_address(d['public_key'].encode('hex')), 'nTime': d['nTime'], 'nVersion': d['nVersion'], 'public_key_hex': d['public_key']})
            except:
                json_db['pool'].append({'n': d['n'], 'addr': public_key_to_bc_address(
                    d['public_key']), 'nTime': d['nTime'], 'nVersion': d['nVersion'], 'public_key_hex': d['public_key'].encode('hex')})

        elif type == "acc":
            json_db['acc'] = d['account']
            sys.stdout.write("Account %s (current key: %s)" % (
                d['account'], public_key_to_bc_address(d['public_key'])))

        elif type == "acentry":
            json_db['acentry'] = (d['account'], d['nCreditDebit'], d['otherAccount'], time.ctime(
                d['nTime']), d['n'], d['comment'])
            ...
            crypted = True
            compressed = d['public_key'][0] != '\04'
            json_db['keys'].append({'pubkey': d['public_key'].encode('hex'), 'addr': public_key_to_bc_address(
                d['public_key']), 'encrypted_privkey':  d['encrypted_private_key'].encode('hex_codec'), 'compressed': compressed})

        elif type == "mkey":
            json_db['mkey']['nID'] = d['nID']
            json_db['mkey']['encrypted_key'] = d['encrypted_key'].encode(
                'hex_codec')
            ...
            if passphrase:
                res = crypter.SetKeyFromPassphrase(
                    passphrase, d['salt'], d['nDerivationIterations'], d['nDerivationMethod'])
                if res == 0:
...
if __name__ == '__main__':
    ...
    for i in range(1, len(sys.argv)):
        ...
        if not crypted:
            print >> sys.stderr, "%s : this wallet is not encrypted" % os.path.basename(
                filename)
            continue
        ...
        sys.stdout.write("$bitcoin$%s$%s$%s$%s$%s$%s$%s$%s$%s\n" %
                         (len(cry_master), cry_master, len(cry_salt),
                          cry_salt, cry_rounds, len(
                              ckey), ckey, len(public_key),
                          public_key))
